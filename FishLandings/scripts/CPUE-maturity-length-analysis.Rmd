...
data$catch_per_trap <- ifelse(data$catch_per_trap > 40, NA, data$catch_per_trap)
...
data %>%
  ...
  ggplot(aes(x=trap_type, y=richness_trap, color=trap_type)) +
  scale_color_manual(values = c( "#387780", "#FF8C61")) +
  scale_fill_manual(values = c( "#387780", "#FF8C61")) +
  theme_bw() + xlab("") + ylab("Richness (# of unique species trap-1)") +
...
nut_biomassdf <- nut_biomassdf %>%
  ...
  mutate(group_Calcium = (W_g/100)*Calcium_mgPer100g,
         group_Iron = (W_g/100)*Iron_mgPer100g,
         group_Omega3 = (W_g/100)*Omega3_gPer100g,
         group_Protein = (W_g/100)*Protein_gPer100g,
         group_VitaminA = (W_g/100)*VitaminA_ugPer100g,
         group_Selenium = (W_g/100)*Selenium_ugPer100g,
         group_Zinc = (W_g/100)*Zinc_mgPer100g) %>% 
  # calculating total nutrient for each survey 
  group_by(survey_id) %>%
  mutate(total_Calcium = sum(group_Calcium)/total_traps_collected,
         total_Iron = sum(group_Iron)/total_traps_collected,
         total_Omega3 = sum(group_Omega3)/total_traps_collected,
         total_Protein = sum(group_Protein)/total_traps_collected,
         total_VitaminA = sum(group_VitaminA)/total_traps_collected,
         total_Selenium = sum(group_Selenium)/total_traps_collected,
         total_Zinc = sum(group_Zinc)/total_traps_collected) %>% ungroup()
