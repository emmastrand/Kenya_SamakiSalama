...
```{r, message=FALSE, warning=FALSE}
# read in excel file
## 4539 rows x 25 variables (this should match the final version of QC script)
...
## unmodified: 3,993 rows from 591 surveys 
...
```{r}
## PRE FILTER 4,539 rows
## POST FILTER 4,529 rows (confirmed only the below was taken outs)
data <- data %>% 
...
range(data2$CPUE) ## 0.25 - 16.28 fish per trap 
...
ggsave(file="output/CPUE.png", CPUE_plot, width = 4, height = 4, units = c("in"))
```
...
## p-value = 0.01627; ratio = 0.7219532
t.test(CPUE~trap_type, data = data2_CPUE, var.equal = FALSE)
```


left with the above looking good. need to replace statistics in spreadsheet table. 6/21/2023 continue tomorrow. 
...
data_summary <- data %>% select(survey_id, trap_type) %>% distinct()
...
# modified_trap_df_summary <- modified_trap_df %>% select(survey_id, trap_type) %>% 
#   distinct() %>% subset(trap_type == "Control")
# 
# control_no <- modified_trap_df %>% subset(trap_type == "Experimental")
...
gilligan_profit <- read.csv("data/SpeciesData_GatedTraps_Galligan_edited.csv", header=TRUE, sep = ",") %>%
  dplyr::select(Species, Bycatch, Price_KSHPerkg) %>% dplyr::rename(scientific_name = Species)
