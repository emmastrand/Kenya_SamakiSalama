---
title: "Analysis of Fishing Landings dataset - household groups"
...
```{r}
summary <- summary %>% filter(mean_lengthdist > -20)
...
Testing individual geom jitter to see if it's worth it

```{r}
summary_subset <- summary_stats %>% subset(measurement == "Distance from Lmat (cm)") ### CHANGE VARIABLE HERE

jitter_plot <- summary2 %>% subset(measurement == "Distance from Lmat (cm)") %>% ### CHANGE VARIABLE HERE
  filter(value > -20) %>%
  ggplot(., aes(x=household_group_collapsed, y=value, color=household_group_collapsed)) + 
  geom_jitter(size=0.6, alpha=0.1, width=0.1) + theme_bw() +
  scale_color_manual(values = c("#11BF66", "#E78818", "#48B5B7")) +
  ...
  theme(#strip.background = element_blank(),
    ...
    geom_point(data=summary_subset, aes(x=household_group_collapsed, y=value), size=1, position=position_dodge(0.3), shape=21)

ggsave(jitter_plot, file="household groups/DistancefromLmat_jitter.png", width=2.5, height=3, units=c("in")) ### CHANGE VARIABLE HERE
```
...
```{r}
aov <- aov(mean_lengthdist~household_group_collapsed, data=summary) ## CHANGE VARIABLE HERE
Anova(aov, type = "III")
TukeyHSD(aov)
```
