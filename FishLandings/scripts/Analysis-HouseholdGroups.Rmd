...
```{r}
summary_subset <- summary_stats %>% subset(measurement == "Length (cm)") ### CHANGE VARIABLE HERE

jitter_plot <- summary2 %>% subset(measurement == "Length (cm)") %>% ### CHANGE VARIABLE HERE
...
ggsave(jitter_plot, file="household groups/Length_jitter.png", width=3, height=4, units=c("in")) ### CHANGE VARIABLE HERE
```
...
```{r}
aov <- aov(Mean_length_cm~household_group_collapsed, data=summary) ## CHANGE VARIABLE HERE
Anova(length_aov, type = "III")
TukeyHSD(length_aov)
```
