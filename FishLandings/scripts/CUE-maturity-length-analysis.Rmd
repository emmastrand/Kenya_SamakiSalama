...
Read in data from fishbase. 
```{r}
biomass_fb <- read_excel("data/fishbase.xlsx", sheet = "biomass") %>% #read in excel file 
  select(scientific_name, a, b)

biomass_df <- data %>% filter(!is.na(number_of_fish)) %>%
  subset(trap_type == "MODIFIED" | trap_type == "UNMODIFIED") %>%
  select(-enumerator, -landing_site, -BMU, -fisher_id, -fisher_phone, -household_id, -date_set_dd_mm_yyyy,
         -`time_set_24hh:mm`, -date_collected_dd_mm_yyyy, -`time_collected_24hh:mm`, -date_time_set, -date_time_collected) 
```
Left off at merging the 2 dataframes and then uncount so each fish observation is a row
