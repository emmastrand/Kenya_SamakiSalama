...
5. In the catch information section, under "SPECIES/scientific name", can you see #1 and #2 and let me know your thoughts on that?  
6.  
...
Converting the written in values to ranges. 

```{r}
## converting the numerical values to ranges 
# isolating just the numerical values 
numerical_values <- df %>% 
  filter(!str_detect(length_cm, ">") & !str_detect(length_cm, "<") & !str_detect(length_cm, "-")) %>%
  filter(!length_cm == "NA")

# converting those to numerical values 
numerical_values$length_cm <- as.numeric(numerical_values$length_cm)

# converting those to be ranges 
numerical_values <- numerical_values %>%
  mutate(length_corrected = if_else(length_cm
    length_cm
  ))

## double checking correct ranges here
ranges <- df %>% 
  filter(str_detect(length_cm, ">") | str_detect(length_cm, "<") | str_detect(length_cm, "-") | length_cm == "NA")

unique(sort(ranges$length_cm))
```

left off at converting the numerical values to be within a bin... line 467 
