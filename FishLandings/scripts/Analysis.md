...
``` r
## "26-30" "16-20" "31-35" "11-15" "21-25" "36-40" "41-45" "46-50" "51-60"
...
length_df <- summarySE(data2_length, measurevar = c("median_length"), groupvars = c("trap_type", "survey_id"))
```

    ## Warning in qt(conf.interval/2 + 0.5, datac$N - 1): NaNs produced

``` r
data2_length %>% write.csv("output/length_total.csv")
length_df %>% write.csv("output/length_persurvey.csv")
...
data2 <- data2 %>% 
  ...
  mutate(length_dist = median_length-Lmat_corrected) %>% group_by(survey_id, trap_type) %>%
  mutate(mean_lengthdist = mean(length_dist)) %>% ungroup()
...
data2_maturity <- data2 %>% 
  ...
  group_by(survey_id) %>%
    mutate(total_catch = n()) %>% ungroup() %>%
  group_by(survey_id, maturity) %>%
    mutate(no_maturity = n(),
           per_maturity = no_maturity/total_catch) %>% ungroup()

data2_maturity_grouped <- data2_maturity %>% dplyr::select(survey_id, trap_type, maturity, per_maturity) %>%
  distinct()
```
...
### Average Trophic Level and Diet

``` r
data3 <- data3 %>% group_by(survey_id, trap_type) %>%
  mutate(mean.trophic = mean(TrophLevel, na.rm = TRUE)) %>% ungroup() 

diet <- data3 %>% group_by(survey_id, trap_type, Diet) %>%
  mutate(countper_diet = n()) %>% ungroup() %>%
  select(survey_id, trap_type, BMU, Diet, countper_diet) %>% distinct() %>%
  group_by(survey_id, trap_type) %>% 
  spread(Diet, countper_diet) %>% 
...
```
    ...
    ## • Columns `survey_id`, `trap_type`
    ## ℹ Use `mutate_at(df, vars(-group_cols()), myoperation)` to silence the message.

``` r
diet %>% write.csv("output/group3_diet.csv")
```

### Calculating biomass taken home

``` r
data3 <- data3 %>% group_by(survey_id, trap_type, destination) %>%
  mutate(kg_perdestination = sum(W_kg)) %>% ungroup()

takehome_kg <- data3 %>%
  dplyr::select(survey_id, trap_type, destination, kg_perdestination) %>% 
  distinct() %>% spread(destination, kg_perdestination) %>% 
  dplyr::select(survey_id, trap_type, HOME)

takehome_kg %>% write.csv("output/takehome_kg.csv")
```

### Exporting analyzed dataframes

``` r
## data3 4,151 × 110 pre left join; 4,151 x 111 post left join 
pertrap_final_df <- data3 %>% left_join(., data2_maturity_grouped, by = c("trap_type", "survey_id", "maturity")) %>%
  dplyr::select(survey_id, BMU, landing_site, fisher_id, household_id, 
                                            ...
                                            pertrap_Selenium, pertrap_Zinc, mean.trophic, mean_lengthdist) %>% distinct()
## took out per_maturity for now
